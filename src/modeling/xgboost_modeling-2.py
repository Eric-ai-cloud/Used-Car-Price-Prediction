...
print("=" * 80)
print("🚀 XGBoost 二手车价格预测建模（k=6 精填数据版）")
print("=" * 80)
...
# 保存为CSV
submission.to_csv('submission_xgboost_k6.csv', index=False)

print(f"✅ 预测结果已保存到: submission_xgboost_k6.csv")
...
try:
    model.save_model('xgboost_price_model_k6.json')
    print("✅ 模型已保存到: xgboost_price_model_k6.json")
except Exception as e:
    print(f"⚠️ 模型保存失败: {e}")
    print("💡 使用备用方法保存...")
    # 使用pickle保存
    import pickle
    with open('xgboost_model_k6.pkl', 'wb') as f:
        pickle.dump(model, f)
    print("✅ 模型已使用pickle保存到: xgboost_model_k6.pkl")

# ========================
# 9. 总结
# ========================
print("\n" + "=" * 80)
print("📊 建模总结（k=6 精填数据版）")
print("=" * 80)
print(f"""
✅ 数据处理:
   - 数据来源: train_refined_k6.csv / test_refined_k6.csv（k=6簇内精填后）
...
✅ 输出文件:
   - 预测结果: submission_xgboost_k6.csv
   - 模型文件: xgboost_price_model_k6.json
...
print("=" * 80)
print("🎉 建模完成！可以查看 submission_xgboost_k6.csv 文件了")
print("=" * 80)
