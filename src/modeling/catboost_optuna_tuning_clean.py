"""
CatBoost超参数调优（纯净版）- 不含v_14异常值标识
在方案A优化版基础上，通过Optuna自动搜索最佳超参数组合，但不添加is_v14_outlier特征

优化内容：
1. 合并极小簇（簇4→簇2）
2. 移除4个冗余特征
3. 添加5个新特征（多项式+交互）
4. Optuna超参数搜索（10个超参数）
5. ❌ 不包含is_v14_outlier特征
...
print("=" * 80)
print("🔬 CatBoost超参数调优（纯净版）- 不含v_14标识")
print("=" * 80)
...
print(f"    ✅ 已添加5个新特征（多项式+交互）")

# 1.4 ⭐ 不添加v_14异常值标识特征
print("\n  1.4 v_14异常值标识特征: ❌ 不包含（经验证无效）")
...
try:
    ...
    plt.tight_layout()
    plt.savefig('optuna_optimization_clean.png', dpi=300, bbox_inches='tight')
    print(f"\n✅ 优化过程图表已保存: optuna_optimization_clean.png")
...
output_file = f'submission_catboost_{CLUSTER_VERSION}_optuna_clean.csv'
...
# 保存模型
model_file = f'catboost_model_{CLUSTER_VERSION}_optuna_clean.cbm'
final_model.save_model(model_file)
print(f"✅ 模型已保存: {model_file}")

# 保存最优超参数
import json
optuna_params_file = f'optuna_best_params_{CLUSTER_VERSION}_clean.json'
...
# 与之前模型对比
baseline_rmse_clean = 1215.51  # 方案A优化版（纯净版）
improvement = baseline_rmse_clean - rmse_val
improvement_pct = improvement / baseline_rmse_clean * 100

print(f"\n💡 性能对比（vs 方案A纯净版 RMSE={baseline_rmse_clean:.2f}）:")
print(f"  RMSE变化: {improvement:+.2f}点 ({improvement_pct:+.2f}%)")

if improvement > 0:
    print(f"  ✅ 性能提升！RMSE降低{improvement:.2f}点")
    print(f"  🎯 Optuna超参数调优有效，推荐使用此模型")
elif abs(improvement) < 1:
    print(f"  🟡 性能基本持平（变化<{abs(improvement):.2f}点）")
    print(f"  💡 超参数已优化，但性能提升有限")
else:
...
print(f"  • optuna_optimization_clean.png - 优化过程可视化")

print(f"\n{'='*80}")
print("✅ Optuna超参数调优（纯净版）完成！")
print(f"{'='*80}")
